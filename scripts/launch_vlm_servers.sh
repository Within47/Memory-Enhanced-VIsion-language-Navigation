...
tmux send-keys -t ${session_name}:0.0 "conda activate vlfm && python -m vlfm.vlm.grounding_dino --port ${GROUNDING_DINO_PORT}" C-m
tmux send-keys -t ${session_name}:0.1 "conda activate vlfm && python -m vlfm.vlm.blip2itm --port ${BLIP2ITM_PORT}" C-m
tmux send-keys -t ${session_name}:0.2 "conda activate vlfm && python -m vlfm.vlm.sam --port ${SAM_PORT}" C-m
tmux send-keys -t ${session_name}:0.3 "conda activate vlfm && python -m vlfm.vlm.yolov7 --port ${YOLOV7_PORT}" C-m
